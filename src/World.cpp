...
float Shade(Face face){switch(face){case Face::Up:return 1.0f;case Face::Down:return 0.50f;case Face::North:case Face::South:return 0.80f;default:return 0.70f;}}
...
void World::UseBuiltInLayers(WarningLog& warnings){
    ...
    for(const auto& [name,count]:defaults){
        const BlockId id=blocks_.Find(name);
        if(id!=0)layers_.push_back({id,count});
        else warnings.Add(L"Built-in terrain requires a missing block texture/definition: "+Widen(name));
    }
}

void World::LoadLayerProfile(const std::filesystem::path& file,WarningLog& warnings){
    // flat_overworld.layers is optional. The built-in stack is used when it is absent.
    if(!std::filesystem::is_regular_file(file))return;

    std::ifstream input(file,std::ios::binary);
    if(!input.is_open()){
        warnings.Add(L"Unreadable optional world layer profile: "+AbsoluteForMessage(file));
        return;
    }
    ...
    while(std::getline(input,line)){
        ...
        if(equals==std::string::npos){warnings.Add(L"Invalid world layer line "+std::to_wstring(lineNumber)+L" in "+AbsoluteForMessage(file)+L"; expected block=count");continue;}
        ...
        const BlockId id=blocks_.Find(name);
        if(id==0){warnings.Add(L"World layer profile references unknown block: "+Widen(name));continue;}
        if(count<=0){warnings.Add(L"Non-positive layer count for "+Widen(name)+L" in "+AbsoluteForMessage(file));continue;}
        parsed.push_back({id,count});
    }

    if(!parsed.empty())layers_=std::move(parsed);
    else warnings.Add(L"Optional world layer profile contains no valid layers; built-in terrain remains active: "+AbsoluteForMessage(file));
}
...
std::array<float,3> World::GrassTintForChunk(ChunkCoord coord) const{
    ...
    // Minecraft-style biome lookup coordinates.
    double temperature=0.70+(fraction-0.5)*0.30;
    double humidity=0.75+(0.5-fraction)*0.25;
...
void World::UpdateStreaming(const Vec3& playerPosition,int renderDistance){
    ...
    for(auto it=chunks_.begin();it!=chunks_.end();){
        const int dx=it->first.x-center.x,dz=it->first.z-center.z;
...
BlockId World::GetBlock(int x,int y,int z) const{
    ...
    const ChunkCoord coord{FloorDiv(x,kChunkSize),FloorDiv(z,kChunkSize)};
...
bool World::SetBlock(int x,int y,int z,BlockId id){
    ...
    const int localX=FloorMod(x,kChunkSize),localZ=FloorMod(z,kChunkSize);
    if(it->second->GetLocal(localX,y,localZ)==id)return false;
    it->second->SetLocal(localX,y,localZ,id);
...
std::optional<RayHit> World::Raycast(const Vec3& origin,const Vec3& direction,double maxDistance) const{
    ...
    if(Length(dir)<1e-9)return std::nullopt;
    ...
    const double deltaX=dir.x==0?std::numeric_limits<double>::infinity():std::abs(1.0/dir.x);
    const double deltaY=dir.y==0?std::numeric_limits<double>::infinity():std::abs(1.0/dir.y);
    const double deltaZ=dir.z==0?std::numeric_limits<double>::infinity():std::abs(1.0/dir.z);
    double distance=0;
    while(distance<=maxDistance){
        const BlockId id=GetBlock(cell.x,cell.y,cell.z);
        if(id!=0&&blocks_.Get(id).solid)return RayHit{cell,previous,normal,id,distance};
        previous=cell;
        if(maxX<maxY&&maxX<maxZ){cell.x+=stepX;distance=maxX;maxX+=deltaX;normal={-stepX,0,0};}
        else if(maxY<maxZ){cell.y+=stepY;distance=maxY;maxY+=deltaY;normal={0,-stepY,0};}
        else{cell.z+=stepZ;distance=maxZ;maxZ+=deltaZ;normal={0,0,-stepZ};}
    }
    return std::nullopt;
}
...
MeshData World::BuildMesh(Chunk& chunk) const{
    MeshData mesh;
    mesh.revision=chunk.NextRevision();
    const int dimensions[3]={kChunkSize,kWorldHeight,kChunkSize};
    const ChunkCoord chunkCoord=chunk.Coord();
    const auto grass=GrassTintForChunk(chunkCoord);
    ...
    for(int axis=0;axis<3;++axis){
        ...
        int x[3]={0,0,0},q[3]={0,0,0};q[axis]=1;
        for(x[axis]=-1;x[axis]<dimensions[axis];){
            size_t n=0;
            for(x[v]=0;x[v]<dimensions[v];++x[v])for(x[u]=0;x[u]<dimensions[u];++x[u],++n){
                const BlockId a=x[axis]>=0?worldBlock(x[0],x[1],x[2]):0;
                const BlockId b=x[axis]<dimensions[axis]-1?worldBlock(x[0]+q[0],x[1]+q[1],x[2]+q[2]):0;
                const bool solidA=blocks_.Get(a).solid,solidB=blocks_.Get(b).solid;
                MaskCell cell{};
                if(solidA!=solidB){
                    const bool positive=solidA;
                    const BlockId id=solidA?a:b;
                    const Face face=AxisFace(axis,positive);
                    const auto& definition=blocks_.Get(id);
                    const float shade=Shade(face);
                    float red=shade,green=shade,blue=shade;
                    if(definition.tint[static_cast<size_t>(face)]==TintMode::Grass){red*=grass[0];green*=grass[1];blue*=grass[2];}
                    cell={true,!positive,definition.textureSlices[static_cast<size_t>(face)],PackColor(red,green,blue),face};
                }
                mask[n]=cell;
            }
            ...
            for(int j=0;j<dimensions[v];++j)for(int i=0;i<dimensions[u];){
                ...
                if(!cell.valid){++i;++n;continue;}
                int width=1;while(i+width<dimensions[u]&&mask[n+width]==cell)++width;
                int height=1;bool stop=false;
                while(j+height<dimensions[v]&&!stop){for(int k=0;k<width;++k)if(!(mask[n+k+height*dimensions[u]]==cell)){stop=true;break;}if(!stop)++height;}
                x[u]=i;x[v]=j;
                int du[3]={0,0,0},dv[3]={0,0,0};du[u]=width;dv[v]=height;
                ...
                for(int row=0;row<height;++row)for(int column=0;column<width;++column)mask[n+column+row*dimensions[u]]={};
                i+=width;n+=width;
            }
        }
    }
...
std::vector<std::pair<ChunkCoord,MeshData>> World::BuildDirtyMeshes(size_t budget){
    std::vector<std::pair<ChunkCoord,MeshData>> result;
    result.reserve(budget);
    for(auto& [coord,chunk]:chunks_){if(result.size()>=budget)break;if(chunk->Dirty())result.emplace_back(coord,BuildMesh(*chunk));}
    return result;
}
